...
export const TestimonialsSection = () => {
  ...
  const MobileLayout = () => (
    <div ref={sectionRef} className="py-12 sm:py-16 lg:py-24 px-4 sm:px-6 lg:px-8 xl:px-12 bg-gradient-to-b from-gray-50 to-white">
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 max-w-7xl mx-auto">
          {testimonials.map((testimonial, index) => (
            <motion.div
              ...
              variants={cardVariants}
              whileHover={{ y: -8, transition: { duration: 0.3 } }}
              className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 p-6 lg:p-8 relative"
            >
              <div className="absolute top-0 left-0 w-full h-1">
                <div className={`h-full w-24 ${testimonial.color}`}></div>
              </div>

              <Quote className="text-gray-200 h-10 w-10 lg:h-12 lg:w-12 absolute top-4 lg:top-6 right-4 lg:right-6 rotate-180" />

              <div className="flex space-x-1 mb-4">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`h-4 w-4 lg:h-5 lg:w-5 ${i < testimonial.rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`}
                  />
                ))}
              </div>

              <p className="text-gray-700 relative z-10 mb-6 leading-relaxed text-sm lg:text-base">{testimonial.text}</p>

              <div className="flex items-center mt-6">
                <div
                  className={`h-10 w-10 lg:h-12 lg:w-12 rounded-full flex items-center justify-center text-white text-xs lg:text-sm font-bold shadow-sm border-2 border-white ${testimonial.color}`}
                >
                  {testimonial.initials}
                </div>
                <div className="ml-3 lg:ml-4">
                  <p className="font-semibold text-gray-900 text-sm lg:text-base">{testimonial.name}</p>
                  <p className="text-gray-500 text-xs lg:text-sm">{testimonial.car}</p>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      <div className="md:hidden">
        <div className="relative px-2 sm:px-4">
            ...
            >
              {testimonials.map((testimonial, index) => (
                <div key={index} className="w-full flex-shrink-0 px-2 sm:px-4">
                  <div className="bg-white rounded-xl overflow-hidden shadow-lg p-4 sm:p-6 relative">
                    <div className="absolute top-0 left-0 w-full h-1">
                      <div className={`h-full w-20 sm:w-24 ${testimonial.color}`}></div>
                    </div>

                    <Quote className="text-gray-200 h-6 w-6 sm:h-8 sm:w-8 absolute top-3 sm:top-4 right-3 sm:right-4 rotate-180" />
                    ...
                    <p className="text-gray-700 relative z-10 mb-4 sm:mb-6 text-xs sm:text-sm leading-relaxed">{testimonial.text}</p>

                    <div className="flex items-center mt-4">
                      <div
                        className={`h-8 w-8 sm:h-10 sm:w-10 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm border-2 border-white ${testimonial.color}`}
                      >
                        {testimonial.initials}
                      </div>
          ...
          <button
            onClick={prevTestimonial}
            className="absolute left-1 sm:left-2 top-1/2 -translate-y-1/2 bg-white rounded-full p-1.5 sm:p-2 shadow-md z-10"
            aria-label="Previous testimonial"
          >
          ...
          <button
            onClick={nextTestimonial}
            className="absolute right-1 sm:right-2 top-1/2 -translate-y-1/2 bg-white rounded-full p-1.5 sm:p-2 shadow-md z-10"
            aria-label="Next testimonial"
          >
