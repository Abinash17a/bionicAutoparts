...
const Admin = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/getSubmissionsv3?timestamp=${new Date().getTime()}`);
  ...
  const resetLogoutTimer = useCallback(() => {
    ...
    const newLogoutTimer = setTimeout(() => handleLogoutRef.current?.(), 2 * 60 * 60 * 1000);
  ...
  // Assign the ref after function declaration
  handleLogoutRef.current = handleLogout;

  const handleStatusChange = async (submissionId: string, status: string) => {
    try {
      const response = await fetch('/api/updateSubmissionStatus3', {
        method: 'POST',
...
const handleImageUpload = async (
  ...
  try {
    ...
    const updateRes = await fetch("/api/uploadImagev3", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  }, [isLoggedIn, resetLogoutTimer, logoutTimer, fetchInterval]);
